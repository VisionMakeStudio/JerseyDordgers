...
export function buildGameChangerRecap({details,team,source}){
 const teamName=String(team?.name||'Jersey Dodgers'),opponent=String(details?.opponent_team?.name||'Opponent'),ourScore=Number(details?.score?.team),theirScore=Number(details?.score?.opponent_team);if(!Number.isFinite(ourScore)||!Number.isFinite(theirScore))throw Error('GameChanger has not posted a score for this game yet.');
 const timezone=details.timezone||'America/New_York',date=localDate(details.start_ts,timezone),dateLabel=gameDateLabel(details.start_ts,timezone),won=ourScore>theirScore,lost=ourScore<theirScore;
 ...
 return {title,date,opponent,ourScore,theirScore,source,summary,body,lineScore,boxScore:'',warnings:['Score and inning totals imported. GameChanger keeps its written recap and player box score behind account access, so those can still be pasted manually if desired.']};
}
