...
export const schemas={
 gameRecaps:z.object({id,gameId:optionalId.default(''),title:short.min(1),date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),opponent:short.min(1),ourScore:z.number().int().min(0).max(200).nullable(),theirScore:z.number().int().min(0).max(200).nullable(),summary:text.default(''),body:z.string().max(40000).default(''),lineScore:z.string().max(12000).default(''),boxScore:z.string().max(40000).default(''),source:link.default(''),image:image.default(''),author:short.default(''),published:z.boolean().default(false)}).refine(r=>(r.ourScore===null)===(r.theirScore===null),'Enter both final scores, or leave both blank'),
 settings:z.object({teamName:short.min(1),founded:short,tagline:short,heroTitle:short,heroText:text,heroImage:image,shareImage:image.default(''),seoTitle:short.default(''),seoDescription:short.default(''),logo:image,scriptLogo:image,leagueName:short,leagueLogo:image,leagueJoinUrl:link.default('https://www.primetimebaseballleague.com/teams/?u=PRIMETIMEBASEBALLLEA&s=baseball'),instagram:link,youtube:link.default('https://youtube.com/@JerseyDodgers?si=FP33CxgYEcQ_az5B'),contactEmail:z.union([z.literal(''),z.email()]),applicationEmail:z.union([z.literal(''),z.email()]).default(''),currentSeason:id,championshipTitle:short,championshipText:text,championshipImage:image,tryoutsTitle:short,tryoutsText:text,tryoutsLink:link,sponsorTitle:short,sponsorText:text,footerText:short,creatorName:short.default('Vision Make Studio'),creatorUrl:link.default('https://VisionMakeStudio.com')}),
 ...
 playoffRosters:z.object({id,season:id,player:id,number:z.string().max(10),position:short,active:z.boolean().default(true),source:link.default('')}),
 ...
 playoffStats:z.object({id,season:id,player:id,bat:stats,pitch:stats,fielding:stats,source:short}),
...
export const contentSchema=z.object({
 ...
 playoffRosters:z.array(schemas.playoffRosters).max(5000).default([]),
 jerseyRecords:z.array(schemas.jerseyRecords).max(10000).default([]),
 stats:z.array(schemas.stats).max(10000),
 playoffStats:z.array(schemas.playoffStats).max(10000).default([]),
 ...
 for(const r of d.gameRecaps)if(r.gameId&&!has('games',r.gameId))fail('A game recap references a missing scheduled game');
 ...
 for(const r of d.playoffRosters){if(!has('seasons',r.season)||!has('players',r.player))fail('A playoff roster entry references a missing player or season')}
 ...
 const rosterKeys=d.rosters.map(r=>r.season+':'+r.player);if(new Set(rosterKeys).size!==rosterKeys.length)fail('Only one regular roster entry per player and season is allowed');
 const playoffRosterKeys=d.playoffRosters.map(r=>r.season+':'+r.player);if(new Set(playoffRosterKeys).size!==playoffRosterKeys.length)fail('Only one playoff roster entry per player and season is allowed');
});
