...
export const schemas={
 ...
 media:z.object({id,title:short.min(1),caption:text,image,video:video.default(''),link,category:z.enum(['Photos','News','Video','Instagram']),date:z.string().max(10),photographerName:short.default(''),photographerInstagram:link.default(''),featured:z.boolean().default(false),featureOrder:z.number().int().min(0).max(99).default(0),published:z.boolean()}),
 channels:z.object({id,name:short.min(1),eyebrow:short,description:text,logo:image,link,order:z.number().int().min(0).max(999),active:z.boolean()}),
...
export const contentSchema=z.object({settings:schemas.settings,seasons:z.array(schemas.seasons).max(100),fields:z.array(schemas.fields).max(500),teams:z.array(schemas.teams).max(500),games:z.array(schemas.games).max(5000),players:z.array(schemas.players).max(1000),stats:z.array(schemas.stats).max(10000),standings:z.array(schemas.standings).max(3000),media:z.array(schemas.media).max(2000),channels:z.array(schemas.channels).max(100).default([{id:'youtube',name:'Jersey Dodgers on YouTube',eyebrow:'OFFICIAL YOUTUBE',description:'Introductions, game highlights and team stories.',logo:'',link:'https://youtube.com/@JerseyDodgers?si=FP33CxgYEcQ_az5B',order:1,active:true},{id:'instagram',name:'@jerseydodgers',eyebrow:'OFFICIAL INSTAGRAM',description:'Photos, highlights and game-day posts.',logo:'',link:'https://www.instagram.com/jerseydodgers/',order:2,active:true},{id:'gamechanger',name:'Jersey Dodgers on GameChanger',eyebrow:'LIVE SCORES & STREAMS',description:'Follow the team and open live broadcasts.',logo:'',link:'',order:3,active:true}]),sponsors:z.array(schemas.sponsors).max(300),achievements:z.array(schemas.achievements).max(100),spotlights:z.array(schemas.spotlights).max(1000)}).superRefine((d,ctx)=>{
...
export function publicContent(data){const normalized=contentSchema.parse(data);return {...normalized,media:normalized.media.filter(x=>x.published),channels:normalized.channels.filter(x=>x.active),sponsors:normalized.sponsors.filter(x=>x.active),spotlights:normalized.spotlights.filter(x=>x.published)}}
