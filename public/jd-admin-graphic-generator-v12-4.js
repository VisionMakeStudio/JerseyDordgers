(() => {
  'use strict';
  if (window.__jdGraphicGeneratorV124Loaded) return;
  window.__jdGraphicGeneratorV124Loaded = true;
  ...
  async function removeBackground(){
    ...
    try{
      const img=await loadImage(url),iw=img.naturalWidth||img.width,ih=img.naturalHeight||img.height,canvas=document.createElement('canvas');canvas.width=iw;canvas.height=ih;canvas.getContext('2d').drawImage(img,0,0,iw,ih);
      cutoutCanvas=trimTransparent(canvas);cutoutSource=photoSource;cutoutCache.set(cutoutKey(),cutoutCanvas);
    }finally{URL.revokeObjectURL(url)}
    setBgStatus('PhotoRoom background removal complete. Clean transparent cutout ready.','good');paintPhotoPreview();renderPoster();return cutoutCanvas;
  }
