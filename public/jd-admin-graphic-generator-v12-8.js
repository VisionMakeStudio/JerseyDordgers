(() => {
  ...
  const GENERATOR_VERSION='24.0',MAX_TRANSFER_BYTES=4_000_000;
  ...
  async function useUploadedPhoto(file){
    if(!file)return;if(photoSource?.startsWith('blob:')&&!extraPhotos.some(p=>p.source===photoSource))URL.revokeObjectURL(photoSource);
  ...
  // V22 occasion layouts share the existing photo, cutout, stats and export controls.
  let occasion='potw',extraPhotos=[],opponentLogo=null;
  const studio={title:'GAMEDAY',team:'JERSEY DODGERS',opponent:'OPPONENT',date:'SATURDAY · SEPTEMBER 19',time:'7:00 PM',venue:'FIELD / LOCATION',broadcast:'',season:'2026 SEASON',accent:'#168bff',ink:'#07111f',fade:75,texture:55,shadow:55,font:'auto',opponentScale:1};
  const studioDefaults={...studio};
  const studioNames={potw:'Player of the Week',dark:'Gameday / Blackout',light:'Gameday / Clean White',stats:'Player Stats / Double Exposure'};
  const studioCanvas=document.createElement('canvas');studioCanvas.width=1080;studioCanvas.height=1350;
  function studioText(ctx,text,x,y,width,size,font=CONDENSED_FONT,color='#fff',align='left'){
    ctx.save();ctx.textAlign=align;ctx.fillStyle=color;ctx.font=`700 ${fitText(ctx,String(text||'').toUpperCase(),width,size,8,font,700)}px ${font}`;ctx.fillText(String(text||'').toUpperCase(),x,y);ctx.restore();
  }
  let studioTexture=null;
  function studioSurface(ctx,print){
    if(!studio.texture)return;
    if(!studioTexture){studioTexture=document.createElement('canvas');studioTexture.width=1080;studioTexture.height=1350;const t=studioTexture.getContext('2d');let seed=2313;const rnd=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296};
      for(let i=0;i<34000;i++){const v=rnd()> .5?240:20;t.fillStyle=`rgba(${v},${v},${v},${.08+rnd()*.22})`;t.fillRect(rnd()*1080,rnd()*1350,.5+rnd()*1.5,.5+rnd()*2);}
      for(let i=0;i<420;i++){const x=rnd()*1080,y=rnd()*1350;t.strokeStyle=`rgba(190,199,211,${.025+rnd()*.06})`;t.lineWidth=.5;t.beginPath();t.moveTo(x,y);t.lineTo(x+8+rnd()*90,y-4+rnd()*8);t.stroke();}
    }
    ctx.save();ctx.globalAlpha=studio.texture/100*(print?.72:.6);ctx.drawImage(studioTexture,0,0);
    if(print){ctx.fillStyle='rgba(10,34,58,.17)';for(let y=390;y<1260;y+=9)for(let x=400;x<1080;x+=9){ctx.beginPath();ctx.arc(x,y,.7,0,Math.PI*2);ctx.fill();}}
    ctx.restore();
  }
  function studioPerson(ctx,img,cx,base,height,opacity=1,flip=false){
    if(!img)return;const w=height*(img.naturalWidth||img.width)/(img.naturalHeight||img.height);
    ctx.save();ctx.globalAlpha=opacity;ctx.translate(cx,base);if(flip)ctx.scale(-1,1);
    ctx.filter=playerGrade==='high'?'contrast(1.16) saturate(1.05)':playerGrade==='dramatic'?'contrast(1.08) saturate(.9)':'none';
    ctx.shadowColor=`rgba(0,0,0,${studio.shadow/(occasion==='light'?460:260)})`;ctx.shadowBlur=occasion==='light'?40+studio.shadow*.7:28+studio.shadow*.35;ctx.shadowOffsetY=occasion==='light'?6:10;
    ctx.drawImage(img,-w/2,-height,w,height);ctx.restore();
  }
  function studioPhotos(ctx,kind){
    const main=cutoutCanvas||photoImage;
    if(kind==='stats'){
      const back=extraPhotos[0],img=back?.cutout||back?.img||main;
      if(img){
        const portrait=studioCanvas.getContext('2d');portrait.clearRect(0,0,1080,1350);
        // Anchor the portrait at its top, never at an oversized off-canvas baseline.
        const h=1480*(back?.scale||1),w=h*(img.naturalWidth||img.width)/(img.naturalHeight||img.height);
        const x=760+(back?.x||0)*.45,y=35+(back?.y||0);
        portrait.save();portrait.translate(x,y);if(back?.flip)portrait.scale(-1,1);
        portrait.filter='saturate(.62) contrast(1.08)';portrait.globalAlpha=back?.opacity??.84;
        portrait.drawImage(img,-w/2,0,w,h);portrait.restore();
        portrait.save();portrait.globalCompositeOperation='source-atop';portrait.fillStyle=studio.accent+'38';portrait.fillRect(0,0,1080,1350);portrait.restore();
        // Feather all four edges so neither uploaded rectangles nor cutouts form a box.
        for(const [x0,y0,x1,y1,stops] of [[325,0,1080,0,[[0,0],[.2,1],[.88,1],[1,0]]],[0,0,0,1280,[[0,0],[.09,1],[.64,1],[1,0]]]]){
          const mask=portrait.createLinearGradient(x0,y0,x1,y1);stops.forEach(([at,alpha])=>mask.addColorStop(at,`rgba(0,0,0,${alpha})`));
          portrait.save();portrait.globalCompositeOperation='destination-in';portrait.fillStyle=mask;portrait.fillRect(0,0,1080,1350);portrait.restore();
        }
        ctx.drawImage(studioCanvas,0,0);studioSurface(ctx,true);
      }
    }
    const layer=studioCanvas.getContext('2d');layer.clearRect(0,0,1080,1350);
    const mainX=kind==='stats'?475:kind==='light'?460:540,mainBase=kind==='dark'?970:kind==='stats'?1240:1190,mainH=kind==='dark'?850:kind==='stats'?860:930;
    for(const [i,p] of extraPhotos.entries()){
      if(kind==='stats'&&i===0)continue;
      studioPerson(layer,p.cutout||p.img,540+p.x,mainBase+p.y,mainH*p.scale,p.opacity,p.flip);
    }
    if(main)studioPerson(layer,main,mainX+playerX,mainBase+playerY,mainH*playerScale);
    else studioText(layer,'UPLOAD PLAYER PHOTOS',540,690,580,24,CONDENSED_FONT,kind==='dark'?'#91aec9':'#56738e','center');
    // Fade the actual photo layer, including rectangular uploads, into the background.
    if(studio.fade>0&&kind!=='light'){const g=layer.createLinearGradient(0,mainBase-360*studio.fade/100,0,mainBase+35);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,1)');layer.save();layer.globalCompositeOperation='destination-out';layer.fillStyle=g;layer.fillRect(0,Math.max(0,mainBase-360*studio.fade/100),1080,1350);layer.restore();}
    if(kind==='stats'){const paper=ctx.createLinearGradient(290,0,410,0);paper.addColorStop(0,'#f7f8f6');paper.addColorStop(1,'rgba(247,248,246,0)');ctx.fillStyle=paper;ctx.fillRect(0,0,410,1350);}
    ctx.drawImage(studioCanvas,0,0);
  }
  function studioMatchup(ctx,x,y,ink){
    const home=currentTopLogo();drawImageContain(ctx,home,x-175+topLogoX,y+topLogoY,120*topLogoScale,120*topLogoScale,topLogoOpacity);drawImageContain(ctx,opponentLogo,x+115-60*studio.opponentScale,y+60-60*studio.opponentScale,120*studio.opponentScale,120*studio.opponentScale);
    if(!opponentLogo){ctx.save();ctx.strokeStyle=ink;ctx.lineWidth=2;ctx.strokeRect(x+69,y+17,92,84);studioText(ctx,'OPP',x+115,y+69,72,23,CONDENSED_FONT,ink,'center');ctx.restore();}
    ctx.save();ctx.strokeStyle=studio.accent;ctx.lineWidth=5;ctx.beginPath();ctx.ellipse(x,y+61,46,29,-.18,0,Math.PI*2);ctx.stroke();ctx.restore();studioText(ctx,'VS',x,y+73,70,34,ELECTRIC_FONT,ink,'center');
    studioText(ctx,studio.team,x-110,y+150,270,17,CONDENSED_FONT,ink,'center');studioText(ctx,studio.opponent,x+110,y+150,270,17,CONDENSED_FONT,ink,'center');
  }
  function renderStudio(ctx){
    const label=$('#jd-preview-style-name');if(label)label.textContent=studioNames[occasion]+' · 1080 × 1350';
    const dark=occasion==='dark',stats=occasion==='stats',ink=dark?'#fff':studio.ink;
    const font=studio.font==='auto'?(dark?ELECTRIC_FONT:stats?CONDENSED_FONT:TITLE_FONT):studio.font==='angular'?ELECTRIC_FONT:studio.font==='slab'?CLASSIC_FONT:TITLE_FONT;
    ctx.fillStyle=dark?'#080b10':'#f7f8f6';ctx.fillRect(0,0,1080,1350);
    if(dark){
      const g=ctx.createRadialGradient(530,410,50,530,410,850);g.addColorStop(0,'#283340');g.addColorStop(1,'#030509');ctx.fillStyle=g;ctx.fillRect(0,0,1080,1350);
      ctx.fillStyle=studio.accent;ctx.fillRect(258,208,568,580);
      ctx.save();ctx.strokeStyle='rgba(169,191,214,.10)';ctx.lineWidth=2;
      for(let i=0;i<30;i++){ctx.beginPath();ctx.moveTo(0,370+i*15);ctx.lineTo(540,655+i*5);ctx.lineTo(1080,330+i*16);ctx.stroke();}ctx.restore();
      studioPhotos(ctx,'dark');
      studioSurface(ctx,false);
      const start=730-studio.fade*1.5,fade=ctx.createLinearGradient(0,start,0,850);fade.addColorStop(0,'rgba(0,0,0,0)');fade.addColorStop(.5,'rgba(0,0,0,.74)');fade.addColorStop(1,'#000');ctx.fillStyle=fade;ctx.fillRect(0,start,1080,1350-start);
      studioText(ctx,studio.team,540,76,920,23,CONDENSED_FONT,'#d7e4ef','center');
      studioText(ctx,studio.title,540,965,950,132,font,'#fff','center');studioMatchup(ctx,540,1000,'#fff');
      studioText(ctx,[studio.date,studio.time].filter(Boolean).join(' · '),540,1210,940,25,CONDENSED_FONT,'#fff','center');
      studioText(ctx,[studio.venue,studio.broadcast].filter(Boolean).join(' · '),540,1261,930,20,CONDENSED_FONT,'#b8c6d5','center');
    }else if(!stats){
      studioText(ctx,studio.team,540,73,920,23,CONDENSED_FONT,studio.ink,'center');
      studioText(ctx,studio.title,540,363,1000,300,font,studio.accent,'center');
      // Ground shadow remains separate from the photo and its bottom fade.
      ctx.save();ctx.filter='blur(28px)';ctx.fillStyle=`rgba(18,30,42,${studio.shadow/330})`;
      ctx.beginPath();ctx.ellipse(460+playerX,1195+playerY,185*playerScale,24,0,0,Math.PI*2);ctx.fill();ctx.restore();
      studioPhotos(ctx,'light');if(extraPhotos.length){ctx.fillStyle='rgba(247,248,246,.94)';roundedRect(ctx,646,625,403,395,12);ctx.fill();}studioMatchup(ctx,845,750,studio.ink);
      studioText(ctx,studio.date,835,683,340,23,CONDENSED_FONT,studio.ink,'center');studioText(ctx,studio.time,835,720,300,22,CONDENSED_FONT,studio.ink,'center');
      studioText(ctx,studio.venue,835,948,335,19,CONDENSED_FONT,studio.ink,'center');studioText(ctx,studio.broadcast,835,980,330,16,CONDENSED_FONT,studio.ink,'center');
      studioText(ctx,textState.playerName,540,1275,930,27,CONDENSED_FONT,studio.accent,'center');
    }else{
      const wash=ctx.createLinearGradient(325,0,1080,1200);wash.addColorStop(0,'#e7eef2');wash.addColorStop(.48,studio.accent);wash.addColorStop(1,studio.ink);ctx.fillStyle=wash;ctx.fillRect(325,0,755,1350);
      ctx.save();ctx.strokeStyle='rgba(221,237,250,.16)';ctx.lineWidth=2;ctx.font=`700 580px ${TITLE_FONT}`;ctx.strokeText(String(textState.jerseyNumber||'D').replace(/^#/,''),450,570);ctx.restore();
      studioSurface(ctx,true);
      studioPhotos(ctx,'stats');
      ctx.save();ctx.translate(60,1200);ctx.rotate(-Math.PI/2);studioText(ctx,textState.playerName,0,0,1070,48,font,studio.accent);ctx.restore();
      drawImageContain(ctx,currentTopLogo(),110+topLogoX,37+topLogoY,205*topLogoScale,114*topLogoScale,topLogoOpacity);
      studioText(ctx,studio.season,123,217,220,33,font,studio.accent);studioText(ctx,textState.mainTitle==='PLAYER'?'SEASON STATS':textState.mainTitle,123,253,226,20,CONDENSED_FONT,studio.ink);
      posterStats().slice(0,5).forEach((stat,i)=>{const y=343+i*139;studioText(ctx,stat.value,123,y,205,75,font,studio.accent);studioText(ctx,stat.label,125,y+35,210,22,CONDENSED_FONT,studio.ink);});
      studioText(ctx,textState.weekLabel,125,1136,216,19,CONDENSED_FONT,studio.ink);
      studioText(ctx,textState.tagline,125,1178,216,15,CONDENSED_FONT,studio.ink);
      ctx.fillStyle=studio.ink;ctx.fillRect(0,1304,1080,46);studioText(ctx,textState.website,540,1335,920,17,CONDENSED_FONT,'#fff','center');
    }
    // Existing custom logo placement remains available in game-day layouts.

    if(!dark&&studio.texture>0){ctx.save();ctx.globalCompositeOperation='multiply';ctx.fillStyle=`rgba(14,38,67,${studio.texture/1600})`;let seed=17;for(let i=0;i<studio.texture*80;i++){seed=(seed*1664525+1013904223)>>>0;const x=seed%1080;seed=(seed*1664525+1013904223)>>>0;ctx.fillRect(x,seed%1350,1,2)}ctx.restore();}
    generated=true;const dl=$('#jd-download-graphic');if(dl)dl.disabled=false;
  }
  function studioExtraUI(){
    const list=$('#jd-extra-list');if(!list)return;
    list.innerHTML=extraPhotos.map((p,i)=>`<div class="jd-extra-card"><strong>${i===0?'Second / background photo':'Additional photo '+(i+1)} · ${esc(p.name)}</strong><div class="jd-extra-actions"><button type="button" data-extra-main="${i}">Make main / remove background</button><button type="button" data-extra-delete="${i}">Remove</button></div>${[['x','Left / right',-540,540,1],['y','Up / down',-700,400,1],['scale','Size',.3,2,.01],['opacity','Opacity',.1,1,.01]].map(([k,label,min,max,step])=>`<label>${label}<input type="range" data-extra="${i}" data-prop="${k}" min="${min}" max="${max}" step="${step}" value="${p[k]}"></label>`).join('')}<label><input type="checkbox" data-extra-flip="${i}" ${p.flip?'checked':''}> Flip horizontally</label></div>`).join('');
    list.querySelectorAll('[data-extra]').forEach(el=>el.oninput=()=>{extraPhotos[+el.dataset.extra][el.dataset.prop]=+el.value;renderPoster()});
    list.querySelectorAll('[data-extra-flip]').forEach(el=>el.onchange=()=>{extraPhotos[+el.dataset.extraFlip].flip=el.checked;renderPoster()});
    list.querySelectorAll('[data-extra-delete]').forEach(el=>el.onclick=()=>{const [p]=extraPhotos.splice(+el.dataset.extraDelete,1);if(p.source?.startsWith('blob:'))URL.revokeObjectURL(p.source);studioExtraUI();renderPoster()});
    list.querySelectorAll('[data-extra-main]').forEach(el=>el.onclick=()=>{
      const p=extraPhotos[+el.dataset.extraMain];const previous={img:photoImage,cutout:cutoutCanvas,source:photoSource,name:photoLabel,method:cutoutMethod};
      photoImage=p.img;cutoutCanvas=p.cutout||null;photoSource=p.source;photoLabel=p.name;cutoutSource=cutoutCanvas?photoSource:'';cutoutMethod=p.method||'';
      if(previous.img)Object.assign(p,previous);else extraPhotos.splice(+el.dataset.extraMain,1);
      paintPhotoPreview();studioExtraUI();renderPoster();setBgStatus('This is now the main photo. Use Free Local AI or PhotoRoom above, then swap it back if needed.','');
    });
  }
  function installStudioControls(){
    const sections=Array.from($('.jd-graphic-controls').children);
    const purpose=document.createElement('label');purpose.innerHTML=`Graphic occasion<select id="jd-occasion">${Object.entries(studioNames).map(([v,n])=>`<option value="${v}" ${v===occasion?'selected':''}>${n}</option>`).join('')}</select>`;sections[0].prepend(purpose);
    const gamePicker=document.createElement('label');gamePicker.innerHTML='<span>Fill from saved schedule (optional)</span><select id="jd-studio-game"><option value="">Choose a game…</option>'+Array.from(data?.games||[]).sort((a,b)=>String(a.date).localeCompare(String(b.date))).map(g=>`<option value="${esc(g.id)}">${esc(g.rescheduledDate||g.date)} · ${esc(data.teams?.find(t=>t.id===g.opponent)?.name||'Opponent')}</option>`).join('')+'</select>';sections[0].append(gamePicker);
    let gameSelection=0;
    $('#jd-studio-game').onchange=async e=>{const token=++gameSelection,g=data.games.find(g=>g.id===e.target.value);if(!g)return;const team=data.teams?.find(t=>t.id===g.opponent);studio.opponent=team?.name||'OPPONENT';studio.date=g.rescheduledDate||g.date||'';studio.time=g.rescheduledTime||g.time||'';studio.venue=data.fields?.find(f=>f.id===g.field)?.name||'';studio.season=data.seasons?.find(s=>s.id===g.season)?.name||studio.season;opponentLogo=null;
      $$('[data-studio]').forEach(el=>{el.value=studio[el.dataset.studio]});renderPoster();if(team?.logo)try{const image=await loadImage(team.logo);if(token===gameSelection){opponentLogo=image;renderPoster()}}catch{setBgStatus('Game details loaded. Upload the opponent logo in Logo if the saved image is unavailable.','')}
    };
    const details=document.createElement('div');details.className='jd-studio-fields';details.innerHTML=`<h3>Game details & layout</h3>${[['title','Gameday headline'],['team','Home team'],['opponent','Opponent'],['date','Game date'],['time','First pitch / time'],['venue','Field / location'],['broadcast','Watch / broadcast'],['season','Stats season heading']].map(([k,label])=>`<label>${label}<input data-studio="${k}" value="${esc(studio[k])}"></label>`).join('')}<label>Accent color<input type="color" data-studio="accent" value="${studio.accent}"></label><label>Dark ink color<input type="color" data-studio="ink" value="${studio.ink}"></label><label>Headline font<select data-studio="font">${[['auto','Template default'],['angular','Angular sports'],['condensed','Tall condensed'],['slab','Heritage slab']].map(([v,t])=>`<option value="${v}" ${studio.font===v?'selected':''}>${t}</option>`).join('')}</select></label>`;
    $('.jd-details-body',sections[4]).prepend(details);
    const photos=document.createElement('div');photos.className='jd-studio-fields';photos.innerHTML='<h3>Multiple photos</h3><p>Add up to three extra photos for Gameday or Stats. Transparent cutouts give the closest match to your references. Use “Make main” to remove any photo’s background with the existing tools. Photos stay in this browser session.</p><label>Add photos<input id="jd-extra-upload" type="file" accept="image/*" multiple></label><div id="jd-extra-list"></div>';sections[1].append(photos);
    const logo=document.createElement('div');logo.className='jd-studio-fields';logo.innerHTML='<h3>Opponent logo</h3><label>Upload opponent logo<input id="jd-opponent-logo" type="file" accept="image/*"></label><button type="button" id="jd-opponent-reset">Clear opponent logo</button>';sections[2].append(logo);
    const effects=document.createElement('div');effects.className='jd-studio-fields';effects.innerHTML='<h3>Gameday / Stats finish</h3>'+[['fade','Photo bottom fade'],['texture','Paper / grain texture'],['shadow','Photo shadow']].map(([k,label])=>`<label>${label}<input type="range" data-studio="${k}" min="0" max="100" step="1" value="${studio[k]}"></label>`).join('');$('.jd-edge-intensity').append(effects);
    $('#jd-occasion').onchange=e=>{occasion=e.target.value;$('#jd-preview-style-name').textContent=occasion==='potw'?graphicStyle.toUpperCase()+' · 1080 × 1350':studioNames[occasion]+' · 1080 × 1350';renderPoster()};
    $$('[data-studio]').forEach(el=>el.addEventListener('input',()=>{studio[el.dataset.studio]=el.type==='range'?+el.value:el.value;renderPoster()}));
    $('#jd-extra-upload').onchange=async e=>{
      const files=Array.from(e.target.files||[]),room=3-extraPhotos.length;if(files.length>room)setBgStatus('Up to three additional photos are supported. Only the available slots will be added.','');
      for(const file of files.slice(0,room)){const source=URL.createObjectURL(file);try{const img=await loadImage(source);extraPhotos.push({img,source,name:file.name,cutout:null,x:extraPhotos.length%2?-235:235,y:20,scale:.85,opacity:1,flip:false})}catch(err){URL.revokeObjectURL(source);setBgStatus(err.message,'error')}}
      e.target.value='';studioExtraUI();renderPoster();
    };
    $('#jd-opponent-logo').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;const source=URL.createObjectURL(file);try{opponentLogo=await loadImage(source);renderPoster()}catch(err){setBgStatus(err.message,'error')}finally{URL.revokeObjectURL(source)}};
    const opponentSize=document.createElement('label');opponentSize.innerHTML=`Opponent logo size<input type="range" min="0.5" max="1.6" step="0.01" value="${studio.opponentScale}">`;logo.append(opponentSize);opponentSize.querySelector('input').oninput=e=>{studio.opponentScale=+e.target.value;renderPoster()};
    $('#jd-opponent-reset').onclick=()=>{opponentLogo=null;renderPoster()};studioExtraUI();
  }
  ...
  function renderPoster(){
    ...
    if(occasion!=='potw'){renderStudio(ctx);return}
  ...
  function renderControls(){
    ...
    installStudioControls();installMobileEditor();bindControls();renderStatRows();paintPhotoPreview();syncBrandingControls();renderPoster();watchPreviewSize();
  }
  ...
  function bindControls(){
    ...
    $('#jd-generate-graphic')?.addEventListener('click',async()=>{const b=$('#jd-generate-graphic'),safari=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),downloadWindow=safari?window.open('about:blank','jdGraphicDownload'):null;try{b.disabled=true;b.textContent='Generating…';if($('#jd-auto-remove-bg')?.checked&&photoImage&&cutoutSource!==photoSource){if(autoRemovalMethod==='photoroom')await removeBackground();else await removeBackgroundLocal();}renderPoster();downloadPoster(downloadWindow);setStatus(studioNames[occasion]+' graphic generated and downloaded.')}catch(e){try{downloadWindow?.close()}catch{}setBgStatus(e.message,'error');setStatus(e.message,true)}finally{b.disabled=false;b.textContent='Generate & Download PNG'}});
    $('#jd-download-graphic')?.addEventListener('click',()=>{try{downloadPoster()}catch(e){setStatus(e.message,true)}});$('#jd-reset-graphic')?.addEventListener('click',async()=>{extraPhotos.forEach(p=>{if(p.source?.startsWith('blob:'))URL.revokeObjectURL(p.source)});extraPhotos=[];opponentLogo=null;Object.assign(studio,studioDefaults);occasion='potw';cutoutCanvas=null;cutoutSource='';graphicStyle='editorial';signatureMode='stylized';signatureImage=null;signatureSource='';signatureScale=1;signatureGap=2;signatureLastX=38;signatureRotation=-4;signatureNumberX=18;signatureNumberY=126;topLogoMode='script';topLogoCustom=null;topLogoSource='';topLogoScale=1;topLogoX=0;topLogoY=0;topLogoOpacity=.96;resetStyleStates();resetStats();resetText();renderControls();await useProfilePhoto();setBgStatus('Graphic reset. Free Local AI is the default; PhotoRoom HQ only runs when you choose it.','')});
  ...
  function posterFilename(){const p=selectedPlayer(),safe=String(p?.name||'player').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');return `jersey-dodgers-${occasion==='potw'?'player-of-the-week-'+graphicStyle:occasion==='stats'?'player-stats':'gameday-'+occasion}-${safe||'player'}.png`}
