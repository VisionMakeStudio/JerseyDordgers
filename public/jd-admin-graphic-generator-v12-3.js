(() => {
  'use strict';
  if (window.__jdGraphicGeneratorV123Loaded) return;
  window.__jdGraphicGeneratorV123Loaded = true;
  ...
  const SIGNATURE_FONT="'Allura', 'Brush Script MT', cursive";
  ...
  function defaultText(){
    const p=rosterPlayer(),first=String(p.name||'PLAYER').trim().split(/\s+/)[0]||'PLAYER';
    return {
  ...
  async function removeBackground(){
    ...
    const max=1400,ratio=Math.min(1,max/Math.max(photoImage.naturalWidth||photoImage.width,photoImage.naturalHeight||photoImage.height));
    ...
    const input=document.createElement('canvas');input.width=w;input.height=h;const ictx=input.getContext('2d',{willReadFrequently:true});ictx.drawImage(photoImage,0,0,w,h);
    ...
    const result=await new Promise(async(resolve,reject)=>{
      ...
      segmenter.onResults(results=>{clearTimeout(timer);try{
        ...
        const maskData=mctx.getImageData(0,0,w,h).data,src=ictx.getImageData(0,0,w,h),count=w*h,prob=new Float32Array(count);
        for(let i=0,p=0;i<maskData.length;i+=4,p++)prob[p]=Math.max(maskData[i],maskData[i+1],maskData[i+2])/255;
        const out=document.createElement('canvas');out.width=w;out.height=h;const octx=out.getContext('2d'),pixels=octx.createImageData(w,h),low=.60,high=.80;
        for(let y=0;y<h;y++)for(let x=0;x<w;x++){
          const pidx=y*w+x,i=pidx*4,p=prob[pidx];
          let eroded=p;
          if(x>0)eroded=Math.min(eroded,prob[pidx-1]);if(x<w-1)eroded=Math.min(eroded,prob[pidx+1]);if(y>0)eroded=Math.min(eroded,prob[pidx-w]);if(y<h-1)eroded=Math.min(eroded,prob[pidx+w]);
          const q=p*.48+eroded*.52;let a=clamp((q-low)/(high-low),0,1);a=a*a*(3-2*a);
          let r=src.data[i],g=src.data[i+1],b=src.data[i+2];
          if(a>0&&a<.96){
            let best=p,bestI=pidx;
            for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){if(!dx&&!dy)continue;const nx=x+dx,ny=y+dy;if(nx<0||ny<0||nx>=w||ny>=h)continue;const ni=ny*w+nx;if(prob[ni]>best){best=prob[ni];bestI=ni}}
            const bi=bestI*4,blend=(1-a)*.72;r=Math.round(r*(1-blend)+src.data[bi]*blend);g=Math.round(g*(1-blend)+src.data[bi+1]*blend);b=Math.round(b*(1-blend)+src.data[bi+2]*blend);
          }
          pixels.data[i]=r;pixels.data[i+1]=g;pixels.data[i+2]=b;pixels.data[i+3]=Math.round(src.data[i+3]*a);
        }
    ...
    cutoutCanvas=result;cutoutSource=photoSource;setBgStatus('Background removed. Edge treatment can hide any remaining fringe.','good');paintPhotoPreview();renderPoster();return result;
  }
  ...
  function drawSignature(ctx){
    ...
    let x=graphicStyle==='classic'?585:42,y=graphicStyle==='electric'?1040:1050,w=graphicStyle==='classic'?290:300,h=120;
    if(graphicStyle==='editorial'){x=42;y=1010;w=300;h=145}
    if(signatureMode==='upload'&&signatureImage){drawImageContain(ctx,signatureImage,x,y,w,h,.95);ctx.fillStyle='#b9d8ff';ctx.font=`700 24px ${CONDENSED_FONT}`;ctx.fillText(jersey?`#${jersey}`:'',x+20,y+h-8);return}
    ctx.save();ctx.fillStyle=graphicStyle==='electric'?'#9ed7ff':'#d4e3f6';ctx.font=`400 ${graphicStyle==='editorial'?76:64}px ${SIGNATURE_FONT}`;ctx.fillText(String(textState.signatureText||'').trim(),x,y+72);ctx.font=`700 23px ${CONDENSED_FONT}`;ctx.fillStyle='#9dc8ff';ctx.fillText(jersey?`#${jersey}`:'',x+18,y+104);ctx.restore();
  }
  ...
  async function onPlayerChanged(){selectedPlayerId=$('#jd-graphic-player')?.value||'';cutoutCanvas=null;cutoutSource='';resetStyleStates();resetStats();resetText();syncPlacement();renderStatRows();syncTextInputs();await useProfilePhoto()}
