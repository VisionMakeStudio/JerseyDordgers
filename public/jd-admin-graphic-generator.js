(() => {
  ...
  let playerScale=.86,playerX=0,playerY=0,drag=null,playerStyle='dramatic';
  ...
  async function removeBackground(){
    ...
    const result=await new Promise(async(resolve,reject)=>{
      ...
      segmenter.onResults(results=>{clearTimeout(timer);try{const out=document.createElement('canvas');out.width=w;out.height=h;const ctx=out.getContext('2d');ctx.clearRect(0,0,w,h);ctx.drawImage(results.segmentationMask,0,0,w,h);ctx.globalCompositeOperation='source-in';ctx.drawImage(input,0,0,w,h);ctx.globalCompositeOperation='source-over';resolve(trimTransparent(out))}catch(e){reject(e)}});
  ...
  function playerRect(img){
    ...
    const targetH=1040*playerScale,targetW=targetH*(iw/ih),cx=292+clamp(playerX,-150,150),base=1208+clamp(playerY,-120,100);
    return {x:cx-targetW/2,y:base-targetH,w:targetW,h:targetH};
  }
  function drawPlayer(ctx,img){
    if(!img)return;const r=playerRect(img);
    ctx.save();ctx.globalAlpha=.16;ctx.filter='grayscale(1) brightness(.42) contrast(1.2) blur(.3px)';ctx.drawImage(img,r.x-110,r.y+52,r.w*.92,r.h*.92);ctx.restore();
    ctx.save();
    if(playerStyle==='blue'){ctx.shadowColor='rgba(0,132,255,.95)';ctx.shadowBlur=34;ctx.filter='contrast(1.13) saturate(1.22) brightness(1.01)'}
    else if(playerStyle==='clean'){ctx.shadowColor='rgba(0,0,0,.65)';ctx.shadowBlur=22;ctx.filter='contrast(1.05) saturate(1.05)'}
    else{ctx.shadowColor='rgba(0,96,220,.72)';ctx.shadowBlur=26;ctx.filter='contrast(1.12) saturate(1.16) brightness(.99)'}
    ctx.drawImage(img,r.x,r.y,r.w,r.h);ctx.restore();
    const fade=ctx.createLinearGradient(0,890,0,1210);fade.addColorStop(0,'rgba(4,10,17,0)');fade.addColorStop(1,'rgba(4,10,17,.72)');ctx.fillStyle=fade;ctx.fillRect(0,850,580,360);
  }
  ...
  async function onPlayerChanged(){selectedPlayerId=$('#jd-graphic-player')?.value||'';playerScale=.86;playerX=0;playerY=0;resetStats();resetText();syncPlacement();renderStatRows();syncTextInputs();await useProfilePhoto()}
  ...
  function renderControls(){
    ...
    content.innerHTML=`<div class="jd-graphic-shell"><div class="jd-graphic-intro"><div><span class="eyebrow">SOCIAL GRAPHICS</span><h2>Player of the Week Generator</h2><p>Choose the player, photo, 3–5 featured stats and text. The preview stays visible while you work.</p></div><span class="jd-graphic-size">1080 × 1350 · 4:5</span></div><div class="jd-graphic-layout">
      <div class="jd-graphic-controls">
        ...
        <section><div class="jd-section-title"><div><h3>3. Featured stats</h3><p>Use 3–5 stats. Pick a saved stat, then edit the value or label if this week needs something different.</p></div></div><div id="jd-graphic-stats"></div></section>
        ${textEditor()}
  ...
  function bindControls(){
    ...
    $('#jd-reset-placement')?.addEventListener('click',()=>{playerScale=.86;playerX=0;playerY=0;syncPlacement();renderPoster()});
    $('#jd-generate-graphic')?.addEventListener('click',async()=>{const b=$('#jd-generate-graphic');try{b.disabled=true;b.textContent='Generating…';if($('#jd-auto-remove-bg')?.checked&&photoImage&&cutoutSource!==photoSource)await removeBackground();renderPoster();await downloadPoster();setStatus('Player of the Week graphic generated and downloaded.')}catch(e){setBgStatus(e.message,'error');setStatus(e.message,true)}finally{b.disabled=false;b.textContent='Generate & Download PNG'}});
    $('#jd-download-graphic')?.addEventListener('click',downloadPoster);
    $('#jd-reset-graphic')?.addEventListener('click',async()=>{cutoutCanvas=null;cutoutSource='';playerScale=.86;playerX=0;playerY=0;playerStyle='dramatic';resetStats();resetText();syncPlacement();renderStatRows();syncTextInputs();await useProfilePhoto();setBgStatus('Graphic reset.','')});
  ...
  function downloadPoster(){
    const canvas=$('#jd-potw-canvas');if(!canvas)return Promise.reject(Error('Preview is not ready.'));const p=selectedPlayer();
    return new Promise((resolve,reject)=>canvas.toBlob(blob=>{if(!blob){reject(Error('Could not create the PNG.'));return}const url=URL.createObjectURL(blob),a=document.createElement('a'),safe=String(p?.name||'player').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');a.href=url;a.download=`jersey-dodgers-player-of-the-week-${safe||'player'}.png`;a.style.display='none';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000);resolve()},'image/png'));
  }
  ...
  async function install(){
    ...
    try{
      // Clean up any V12 duplicate buttons before installing exactly one.
      const existing=$$('#jd-graphic-nav',nav);existing.slice(1).forEach(n=>n.remove());if(existing[0])return;
      access=await api('/api/admin/access');if(!access.media)return;
      // Recheck after the async access call so concurrent installs cannot duplicate the item.
      const after=$$('#jd-graphic-nav',nav);if(after.length){after.slice(1).forEach(n=>n.remove());return}
      const b=document.createElement('button');b.id='jd-graphic-nav';b.type='button';b.textContent='Graphic Generator';b.addEventListener('click',openGenerator);const staff=$('#jd-staff-nav'),mobile=nav.querySelector('.admin-mobile-site');nav.insertBefore(b,staff||mobile||null);
  ...
  const observer=new MutationObserver(()=>install());observer.observe(document.documentElement,{childList:true,subtree:true});[400,900,1700,3000].forEach(t=>setTimeout(install,t));
})();
