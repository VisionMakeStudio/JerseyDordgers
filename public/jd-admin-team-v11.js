(() => {
  ...
  async function openStatsLinkImport(){
    await refreshState();
    ...
    read.onclick=async()=>{
      try{
        ...
        rows.forEach((row,i)=>{row.importIndex=i;const same=roster.filter(r=>norm(r.player.name)===norm(row.name));const exact=same.find(r=>!row.number||String(r.number||'').trim()===String(row.number||'').trim());if(exact)matches.set(i,exact.player.id);row.sameRoster=same});
    ...
    apply.onclick=async()=>{
      ...
      try{
        ...
        sessionStorage.setItem(K_ROSTER_PHASE,chosenPhase);closeEditor();sessionStorage.setItem(K_SECTION,'players');setTimeout(()=>location.reload(),300)
      }catch(e){apply.disabled=false;message.textContent=e.message}
    };
  }
